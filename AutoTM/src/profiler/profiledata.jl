...
function possible_configs(node::XNode)
    ...
    unique_groups = unique(x -> x.group, tensors)
    group_indices = [findall(x -> x.group == g) for g in group_indices]
    @show groups_indices
    for indices in group_indices
        filter!( x -> reduce(isequal(x[inidces])), configs)
    end
...
function _setup!(node::nGraph.Node, config::IOConfig, remote_args::Set{nGraph.Node})
    # If this Node is a Param or Result - we need to mark it's output as a remote arg
    # To maintain consistency of the rest of the code, we still mark tne intermediate
    # tensors are persistent.
    if isparam(node) || isresult(node)
        push!(remote_args, node)
    end

    # Standard inner tensor assignment.
